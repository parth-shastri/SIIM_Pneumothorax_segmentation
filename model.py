...
class Unet(Model):
    def __init__(self, name, out_channels=2, filters=(64, 128, 256, 512)):
        ...
        self.conv_tr_1 = layers.Conv2DTranspose(self.filter_list[3], 3, 2, activation="relu", padding="same")
        self.conv_tr_2 = layers.Conv2DTranspose(self.filter_list[2], 3, 2, activation="relu", padding="same")
        self.conv_tr_3 = layers.Conv2DTranspose(self.filter_list[1], 3, 2, activation="relu", padding="same")
        self.conv_tr_4 = layers.Conv2DTranspose(self.filter_list[0], 3, 2, activation="relu", padding="same")
...
if __name__ == "__main__":
    ...
    model = get_model((config.IMG_SIZE, config.IMG_SIZE), 2)
    custom_objects = {"Unet": Unet, "ConvBlock": ConvBlock}

    loss_obj = get_loss(loss_type=config.LOSS_TYPE)
    model.compile(optimizer="adam", loss="sparse_categorical_crossentropy", metrics=[my_iou_metric])
    print(model.summary())
    ...
    with tf.device("/job:localhost/replica:0/task:0/device:GPU:0"):
        ...
        hist = model.fit(train_data.take(5),
                         epochs=config.EPOCHS,
                         validation_data=val_data.take(5),
                         # callbacks=[tensorboard, ckpt],
                         verbose=1)

    # model.save(config.MODEL_DIR)
    #
    # model.save_weights("overfit")
